...
public:
    ...
    {
        // my second fake graphs submit, I don't wanna lose my streak and don't have to waste
        // back to work
        if (n == 0)
            return {};
        if (n == 1)
        ...
        while (!queue.empty())
        {
            res.clear(); // clear vector before we start traversing level by
                         // level.
            int size = queue.size();
            for (int i = 0; i < size; i++)
            {
                int cur = queue.front();
                queue.pop();
                res.push_back(cur); // adding nodes to vector.Goal is to get a
                                    // vector of  just 1 or 2 nodes available.
