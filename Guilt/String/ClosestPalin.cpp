...
public:
    string nearestPalindromic(string n)
    {
        ...
        for (long i : v)
        {
            ...
            if (digits % 2 != 0)
                postfix.pop_back(); //// If the total length is odd number, pop
                                    /// the middle number in postfix
        ...
        {
            if (candidates[i] != num &&
                abs(candidates[i] - num) <
                    mindiff) // Candidate must not be the same number and abs
                             // diff is minm
            {
...
int main()
{
    Solution sol;
    string ans = "123";
    cout << sol.nearestPalindromic(ans);
    return 0;
}
