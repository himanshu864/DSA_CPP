    ...
    vector<int> inorderTraversal(TreeNode *root)
    {
        ...
        while (temp)
        {
            ...
            else
            {
                ...
                while (rightest->right && rightest->right != temp) // move to right most
                    rightest = rightest->right;

                if (rightest->right == temp) // if connection exist
                {
                    rightest->right = NULL;
                    ans.push_back(temp->val); // print node before making connection for preorder
                    temp = temp->right;
                }
                else // else, make connection
                {
                    rightest->right = temp;
                    temp = temp->left;
                }
